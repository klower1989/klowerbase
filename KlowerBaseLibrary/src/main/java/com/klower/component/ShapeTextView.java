...
public class ShapeTextView extends TextView {
	...
	public static GradientDrawable getBgDrawable(int solidColor, int stroke_Width,
			int stroke_Color, int cornesRadius, int shapeType) {
		GradientDrawable gradientDrawable = new GradientDrawable();
		gradientDrawable.setStroke(stroke_Width, stroke_Color, 0, 0);
		// 如果设定的有Orientation 就默认为是渐变色的Button，否则就是纯色的Button
		gradientDrawable.setColor(solidColor);
		if (shapeType != -1) {
			gradientDrawable.setShape(shapeType);
