...
public class GifDecoder extends Thread {
    ...
    private static GifDecoder plugDecoder = null;

    private static GifDecoder climateDecoder = null;

    public static synchronized GifDecoder getPlugInstance(InputStream is, GifAction act) {
        if (plugDecoder == null) {
            plugDecoder = new GifDecoder(is, act);
            plugDecoder.start();
        } else {
            try {
                is.close();
    ...
    public static synchronized GifDecoder getClimateInstance(InputStream is, GifAction act) {
        if (climateDecoder == null) {
            climateDecoder = new GifDecoder(is, act);
            climateDecoder.start();
        } else {
            try {
                is.close();
            } catch (IOException e) {
    ...
    private void setPixels() {
        System.gc();
        int[] dest = null;
        dest = new int[(this.width * this.height) * 4];
        ...
        try {
            this.image = Bitmap
                    .createBitmap(dest, this.width, this.height, Bitmap.Config.ARGB_4444);
    ...
    private void readImage() {
        ...
        if (this.gifFrame == null) {
            this.gifFrame = new GifFrame(this.image, this.delay, this.frameCount);
            this.currentFrame = this.gifFrame;
        } else {
            GifFrame f = this.gifFrame;
            ...
            f.nextFrame = new GifFrame(this.image, this.delay, this.frameCount);
        }
    ...
    private void destroyBitmap() {
        if (null != this.image && !this.image.isRecycled()) {
            this.image.recycle();
        }
    }
}
