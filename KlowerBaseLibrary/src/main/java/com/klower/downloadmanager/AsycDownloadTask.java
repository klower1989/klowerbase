...
package com.klower.downloadmanager;
...
public class AsycDownloadTask extends AsyncTask<DownloadTask, Integer, DownloadTask> {
    ...
    private void SendError(DownloadTask task, Integer code) {
        LogTool.e("Errors happen while downloading.");
    ...
    protected DownloadTask doInBackground(DownloadTask... tasks) {
        if (tasks.length <= 0) {
            LogTool.e("There is no DownloadTask.");
            return null;
        }
        ...
        if (task == null || !task.isValid()) {
            ...
            LogTool.e("The task is not valid,or the url of the task is not valid.");
            return null;
        }
        ...
        try {
            ...
            if (task.getStatus() == DownloadStatus.STATUS_FINISHED && size == range) {
                LogTool.i("The DownloadTask has already been downloaded.");
                return task;
            }
            ...
            while (true) {
                ...
                LogTool.i("HTTP STATUS CODE: " + status);
                ...
                switch (status) {
                    ...
                    case HttpURLConnection.HTTP_PARTIAL:
                        ...
                        if (!TextUtils.isEmpty(transfer_encoding)
                            ...
                            mode = MODE_TRUNKED;
                            LogTool.i("HTTP MODE: TRUNKED");
                        } else {
                            mode = MODE_DEFAULT;
                            LogTool.i("HTTP MODE: DEFAULT");
                        }
                        ...
                        if (!TextUtils.isEmpty(accept_ranges)
                            ...
                            LogTool.i("Accept-Ranges: bytes");
                        } else {
                            range = 0;
                            LogTool.i("Accept-Ranges: none");
                        }
                        break;
                    ...
                    case HttpURLConnection.HTTP_SEE_OTHER:
                        ...
                        LogTool.i("Redirect Url : " + urlString);
                        break;
                    default:
                        success = false;
                        break;
                ...
                if (!redirect) {
                    if (!success) {
                        ...
                        LogTool.e("Http Connection error. ");
                        return null;
                    }
                    LogTool.i("Successed to establish the http connection.Ready to download...");
                    break;
                }
            }
            ...
            if (range == 0) {
                ...
                LogTool.i("The Task is stored in the sqlite.");
            ...
            if (!dir.exists() && !dir.mkdirs()) {
                SendError(task, DownloadException.DOWNLOAD_TASK_FAILED);
                LogTool.e("The directory of the file can not be created!");
                return null;
            }
            task.setStatus(DownloadStatus.STATUS_RUNNING);
            SaveDownloadTask(task, task.getStatus());

            LogTool.i("DownloadTask " + task);
            ...
            while ((nRead = in.read(buffer, 0, 1024)) > 0) {
                ...
                LogTool.i("cur size:" + (curSize) + "    total size:" + (size) + "    cur progress:" + (progress));
            ...
            if (!isFinishDownloading) {
                LogTool.w("The DownloadTask has not been completely downloaded.");
                SaveDownloadTask(task, task.getStatus());
                return null;
            }
            ...
            range = file.length();
            size = task.getSize();
            LogTool.i("range: " + range + " size: " + size);

            if (range != 0 && range == size) {
                LogTool.i("The DownloadTask has been successfully downloaded.");
                task.setFinishTime(System.currentTimeMillis());
                SaveDownloadTask(task, DownloadStatus.STATUS_FINISHED);
                return task;
            } else {
                LogTool.i("The DownloadTask failed to downloaded.");
                SendError(task, DownloadException.DOWNLOAD_TASK_FAILED);
                return null;
            }
    ...
    private static class InternalHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (result == null || result.mTask == null || result.mTask.isCancelled()) {
                LogTool.i("The asyncTask is not valid or cancelled!");
                return;
            }
