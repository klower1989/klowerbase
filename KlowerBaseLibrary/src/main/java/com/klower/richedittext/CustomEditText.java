...
//        		 if (!boldExists){
//        			 boldButtonClick(selStart, selEnd);
//        		 }
//
//        	}
//        }
...
//        		if (!italicsExists){
//       			 italicButtonClick(selStart, selEnd);
//       		 }
//
//        	}
//        }
...
//        		if (!underlinedExists){
//       			 underlineButtonClick(selStart, selEnd);
//       		 }
//        	}
//        }
//    }
    ...
    public void setTextSize(int textSize, int selectionStart, int selectionEnd) {
        ...
        if (selectionEnd > selectionStart) {
            Spannable spannable = this.getText();
            AbsoluteSizeSpan[] appliedStyles = spannable.getSpans(selectionStart, selectionEnd,
            ...
            if (appliedStyles != null && appliedStyles.length > 0) {
                ...
                int beforeColor = 0;
                int afterColor = 0;
                for (AbsoluteSizeSpan foregroundColorSpan : appliedStyles) {
                    if (spannable.getSpanStart(foregroundColorSpan) < selectionStart) {
                        colorStart = spannable.getSpanStart(foregroundColorSpan);
                        beforeColor = foregroundColorSpan.getSize();
                    }
                    if (spannable.getSpanEnd(foregroundColorSpan) > selectionEnd) {
                        colorEnd = spannable.getSpanEnd(foregroundColorSpan);
                        afterColor = foregroundColorSpan.getSize();
                    }
                    spannable.removeSpan(foregroundColorSpan);
                }
                ...
                //
                if (colorStart > -1) {
                    spannable.setSpan(new AbsoluteSizeSpan(beforeColor, true), colorStart,
                            selectionStart,
                            Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
                if (colorEnd > -1) {
                    spannable.setSpan(new AbsoluteSizeSpan(afterColor, true), selectionEnd, colorEnd,
                            Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }

                spannable.setSpan(new AbsoluteSizeSpan(textSize, true), selectionStart, selectionEnd,
                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            } else {
                spannable.setSpan(new AbsoluteSizeSpan(textSize, true), selectionStart, selectionEnd,
                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    private class DWTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable editable) {
            ...
            if (position > 0) {
                ...
                for (int i = 0; i < appliedStyles.length; i++) {
                    ...
                    }else if (appliedStyles[i] instanceof AbsoluteSizeSpan) {
                        if (currentAbsoluteSizeSpan == null) {
                            currentAbsoluteSizeSpan = (AbsoluteSizeSpan) appliedStyles[i];
                        }
                    }
                }
                ...
                handleInsertTextSizeCharacter(editable, position, currentAbsoluteSizeSpan);
            }
        ...
        private void handleInsertTextSizeCharacter(Editable editable, int position, AbsoluteSizeSpan currentForegroundColorSpan) {
            // Handle color
            if (currentForegroundColorSpan != null) {
                if (currentForegroundColorSpan.getSize() != currentTextSize) {
                    int colorStart = editable.getSpanStart(currentForegroundColorSpan);
                    int colorEnd = editable.getSpanEnd(currentForegroundColorSpan);

                    if (position == colorEnd) {
                        AbsoluteSizeSpan nextSpan = getNextTextSizeSpan(editable, position);
                        if (nextSpan != null) {
                            if (currentTextSize == nextSpan.getSize()) {
                                int colorEndNextSpan = editable.getSpanEnd(nextSpan);
                                editable.removeSpan(currentForegroundColorSpan);
                                editable.removeSpan(nextSpan);
                                // set before span
                                editable.setSpan(new AbsoluteSizeSpan(currentForegroundColorSpan.getSize(), true), colorStart,
                                        colorEnd - appendTextLength,
                                        Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                                // set after span
                                editable.setSpan(new AbsoluteSizeSpan(nextSpan.getSize(), true),
                                        position - appendTextLength,
                                        colorEndNextSpan,
                                        Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                                return;
                    ...
                    if (position - appendTextLength < colorEnd && colorStart != colorEnd) {
                        // Cursor in the text's middle with different color
                        int oldColor = currentForegroundColorSpan.getSize();

                        if (colorStart < position - appendTextLength) {
                            // Before inserting text
                            editable.setSpan(new AbsoluteSizeSpan(oldColor, true), colorStart,
                                    position - appendTextLength,
                                    Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                        }

                        // At inserting
                        editable.setSpan(new AbsoluteSizeSpan(currentTextSize, true), position - appendTextLength,
                                position,
                                Spannable.SPAN_EXCLUSIVE_INCLUSIVE);

                        if (position < colorEnd) {
                            // After inserting
                            editable.setSpan(new AbsoluteSizeSpan(oldColor, true), position, colorEnd,
                                    Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                        }
                    } else
                    {
                ...
                editable.setSpan(new AbsoluteSizeSpan(currentTextSize, true),
                        position - appendTextLength, position,
                        Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            }
        }
        ...
        private AbsoluteSizeSpan getNextTextSizeSpan(Editable editable, int position) {
        ...
        public void beforeTextChanged(CharSequence s, int start, int count, int after) {
            ...
            if ((count - after == 1) || (s.length() == 0) && position > 0) { // Delete character
                ...
                removeTextSizeSpan(position, editable);
        ...
        private void removeTextSizeSpan(int position, Editable editable) {
            AbsoluteSizeSpan previousColorSpan = (AbsoluteSizeSpan) getPreviousForegroundColorSpan(editable, position, AbsoluteSizeSpan.class);
            AbsoluteSizeSpan[] appliedStyles = editable.getSpans(position - 1, position, AbsoluteSizeSpan.class);

            if (appliedStyles.length > 0 && appliedStyles[0] != null && previousColorSpan != null
                    && previousColorSpan.getSize() !=  appliedStyles[0].getSize()) {
                AbsoluteSizeSpan colorSpan = (AbsoluteSizeSpan) appliedStyles[0];
                int colorStart = editable.getSpanStart(colorSpan);
                int colorEnd = editable.getSpanEnd(colorSpan);
    ...
    public  void inSertImage(Drawable bitmap, String path){
        getText().insert(getSelectionStart(), "i");
        getText().setSpan(new ImageSpan(bitmap, path), getSelectionStart()-1, getSelectionStart(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        Log.d("customEditor",
                "getText: " + getSpannedText().toString().replace("src = ''", "abcd"));
    }
}
